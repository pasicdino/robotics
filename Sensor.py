...
class WallSensor:
    ...
    def check_intersect(self, walls):
        ...
        for idx, wall in enumerate(walls):
            ...
            if intersection:
                wall_distances[idx] = self.calc_distance(intersection.coords[0],
                                                         self.start_coord)  # calcs distance between intersection and start point of sensor (the robot)
            else:
...
class FeatureSensor:
            ...
            if exact_distance < self.length:
                if self.check_intersect(feature, map_walls):
                    ...
                    relative_bearing = (math.degrees(
                        bearing + self.robot.orientation - math.pi / 2)) % 360  # Bearing relative to robot orientation (-π/2 offset)
    ...
    #def detect_features(self):
       # visible_features = []
       # for feature in self.robot.map.features:
            #distance = self.calc_distance(self.robot.position, feature)
            #if distance <= self.range:
               # bearing = self.calc_bearing(self.robot.position, feature)
               # visible_features.append((feature, distance, bearing))
        #return visible_features

    #def calc_distance(self, start, end):
        #return math.sqrt((end[0] - start[0]) ** 2 + (end[1] - start[1]) ** 2)

    #def calc_bearing(self, start, end):
        #return math.atan2(end[1] - start[1], end[0] - start[0]) - self.robot.orientation